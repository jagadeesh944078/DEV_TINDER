...
- diffrence between js obj vs JSON
- add the express.json() middleware to your app
- make your signup api dynamic to receive data from the end user
- user.findone with duplicate email ids, which email object returned
...
- we have req, res right in request handler that is express given to us in object format to use it.
- the data you sent is part of that req object only.
- to convert data to javascript object we use app.use(express.json())

### Episode 8

- Explore schematype options from the documentation
- add required,min,max,etc..
- add default value
- create custom validate function for gender
- Improve the DB Schema - PUT all approriate validations on each field in Schema
- add timestamps to the userschema
- App API Level Validation on patch request & signup post api
- Data santitizing - Add Api level validation for each field
- Install validator
- Explore Validator library function and use validator function for email,password etc..
...
- create /GET api and check if you get the cookie back
- install json webtoken
- in login api after email & password validation create JWT token. and send it to user in cookies
...
- if user sends any request it will create the connection that is TCP/IP connection once server sends back data connection will be closed.
...
- $or and $and query in mongoose - https://www.mongodb.com/docs/manual/reference/mql/query-predicates/logical/
- schema.pre("save") function
- Read more about indexes in MongoDB
- why do we need index in DB?
- what is advantage and disadvantage of creating index?
- Read the article about compound indexes https://www.mongodb.com/docs/manual/core/indexes/index-types/index-compound/
